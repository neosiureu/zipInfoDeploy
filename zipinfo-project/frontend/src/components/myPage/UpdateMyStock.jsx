...
export default function AddStock() {
  ...
  const [formData, setFormData] = useState({
    ...
    registDate: property.registDate.substring(0, 10),
  ...
  const [previewTumbImgName, setPreviewTumbImgName] = useState(property.imgOriginNames[0]);
  ...
  const [postcode, setPostcode] = useState(property.stockAddress?.split('^^^')[0] || '');
  const [address, setAddress] = useState(property.stockAddress?.split('^^^')[1] || '');
  const [detailAddress, setDetailAddress] = useState(property.stockAddress?.split('^^^')[2] || '');
  ...
  const handleStockInfo = (e) => {
    ...
    switch(name) {
      ...
      case 'stockName': trimmed.length > 0 && trimmed.length < 50 ? 
      setCheckData((prev) => ({
        ...prev,
        stockName: true
      })) :
          ...
          <div className="my-page-stock-section">
            ...
            <div className="my-page-stock-form-group">
              ...
              <div className="my-page-stock-input-row">
                ...
                <select value={formData.stockType} onChange={handleStockType} className="my-page-stock-input-field">
                ...
                <ul>
                  {stockImg.length > 0 ? (
                    ...
                    previewImgName.map((name, idx) => (
                      <li className="imgList" key={idx}>{name}</li>
                    ))
                  )}
                </ul>
                <p className="my-page-stock-image-upload-desc">이미지 파일의 크기는 5MB를 넘으면 안되고 권장되는 크기는 다음과 같습니다.</p>
